...
The reported behavior does not reproduce with the steps currently documented
in the issue. In a GitHub Actions run using `vite-plus@0.2.4`, Bun 1.3.14, and
Node.js 24.18.0, both the control and `vp run` paths received the two OIDC
environment variables:

| Command | `ACTIONS_ID_TOKEN_REQUEST_URL` | `ACTIONS_ID_TOKEN_REQUEST_TOKEN` |
| --- | --- | --- |
| `npm run release` | `present` | `present` |
| `bunx vp run release` | `present` | `present` |

See the successful
[GitHub Actions run](https://github.com/naokihaba/vite-plus-issue-2189-repro/actions/runs/32349539695).
...
the versions from the issue report and reports only whether each variable is
present.
